"""Phase 1 baseline Random Forest model for churn prediction.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train baseline RF model for churn prediction.")
...
def prepare_features(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series, list[str]]:
    ...
    # If raw binary columns exist, convert explicitly to *_yes to keep reporting clear.
    ...
    # If raw state exists, one-hot it with full set of levels.
    ...
    # If raw area_code exists, drop it (baseline policy).
    if "area_code" in X.columns:
        X = X.drop(columns=["area_code"])

    # Drop any area_code one-hot columns.
    ...
    # Last safety: one-hot encode any remaining object/category columns.
    ...
    # Convert boolean to int for a clean numeric matrix.
...
def save_feature_importance_plot(importance_df: pd.DataFrame, path: Path, max_depth: int | None) -> None:
    ...
    plt.barh(top_df["feature"], top_df["importance"], color="#4C78A8")
    plt.xlabel("Feature importance")
    plt.ylabel("Feature")
    plt.title(f"Baseline RF feature importances (max_depth={max_depth})")
    plt.tight_layout()
...
def save_confusion_matrix_plot(
    ...
    ax.set_title(f"Baseline holdout confusion matrix (max_depth={max_depth})")
    plt.tight_layout()
    plt.savefig(path, dpi=160, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    for max_depth in max_depth_values:
        ...
        cv_df.to_csv(args.output_dir / f"cv_f1_scores_baseline_d{tag}.csv", index=False)
        ...
        importance_df.to_csv(args.output_dir / f"feature_importances_baseline_d{tag}.csv", index=False)

        # ===== [PLOT] Ve cac hinh can nop =====
        save_feature_importance_plot(
            importance_df, args.output_dir / f"feature_importance_baseline_d{tag}.png", max_depth
        )
        save_confusion_matrix_plot(
            y_test, test_pred, args.output_dir / f"confusion_matrix_baseline_d{tag}.png", max_depth
        )

        # ===== [SAVE] Luu danh sach feature, model va metrics =====
        (args.output_dir / f"selected_features_baseline_d{tag}.txt").write_text(
            "\n".join(selected_features), encoding="utf-8"
        )

        with open(args.output_dir / f"model_baseline_d{tag}.pkl", "wb") as f:
            pickle.dump(
                {
                    "model": model,
        ...
        with open(args.output_dir / f"metrics_baseline_d{tag}.json", "w", encoding="utf-8") as f:
            json.dump(metrics, f, indent=2)
        write_metrics_txt(metrics, args.output_dir / f"metrics_baseline_d{tag}.txt")

        summary_rows.append(metrics)
        print("-" * 50)
        print(f"Baseline modeling complete (max_depth={max_depth}).")
    ...
    summary_df.to_csv(args.output_dir / "metrics_baseline_by_max_depth.csv", index=False)

    print("Saved summary:", args.output_dir / "metrics_baseline_by_max_depth.csv")
